    ...
    function addFileToZip($path, $zip, $relativePath = '') {
        ...
        while (($filename = readdir($handle)) !== false) {
            if ($filename != '.' && $filename != '..') {
                $relativeTemp = $relativePath == '' ? $filename : $relativePath . '/' . $filename;
                if (is_dir($path . DIRECTORY_SEPARATOR . $filename)) {
                    addFileToZip($path . DIRECTORY_SEPARATOR . $filename, $zip, $relativeTemp);
                } else {
                    $zip->addFile($path. DIRECTORY_SEPARATOR . $filename, $relativeTemp);
                }
            }
        }
...
if (!function_exists('removeFile')) {
    ...
    function removeFile($path) {
        ...
        while(($filename = readdir($handle)) !== false) {
            if ($filename != '.' && $filename != '..') {
                if (is_dir($path . DIRECTORY_SEPARATOR . $filename)) {
                    removeFile($path . DIRECTORY_SEPARATOR . $filename);
