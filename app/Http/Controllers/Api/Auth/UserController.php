...
class UserController extends CommonController
{
    ...
    public function getList()
    {
        ...
        if (!empty($this->params['role_name'])) {
            $role_id = Role::query()->where('name', $this->params['role_name'])->value('id');
            if (!empty($role_id)) {
                $query->from('sy_users as a');
                $query->leftJoin('sy_model_has_roles as b', 'a.id', 'b.model_id');
        ...
        if (!empty($this->params['desc'])) $query->where('desc', 'like', '%' .$this->params['desc'] . '%');
        if (strlen($status)) $query->where('status', $this->params['status']);
        $data   = $query->get();
